...
| 202 | The update was accepted and requires an asynchronous reload. |
| 409 | `If-Match` does not match the current `config_revision`, or the policy does not support the requested transition. |
...
Manual selection is not emulated for policies that do not support it. A
request to an unsupported group returns `409 selection_not_supported`.
...
[`check-nodes.md`](check-nodes.md) for the request and result contract.
